...
- [x] Step 7: Run `make test && make check`, sanitizers, GC stress — all pass. Fixed pre-existing GC bugs: (1) memory leaks in test_chunk.c manual ObjFunction allocation, (2) heap-use-after-free in gc_free_all where free_owned_map called gc_free_object during teardown, (3) GC stress crashes from unpinned method closures in OP_OBJECT_TYPE, (4) gc_pin_value not handling VAL_INSTANCE/VAL_OBJECT_TYPE (their GC-managed sub-objects were unprotected), (5) free_owned_map calling gc_free_object causing dangling pointers. Added gc_is_sweeping(), gc_unpin_n(), changed gc_pin_value to return int pin count. Updated TUTORIAL.md with anonymous objects, objects as expressions, and local type resolution sections. Created examples/objects_as_expressions.cutlet with .expected file. All 150 parser tests, 84 tokenizer tests, 359 compiler tests, 35 VM tests, 50 GC tests, and 32 example tests pass across regular, sanitizer, and GC stress modes.
