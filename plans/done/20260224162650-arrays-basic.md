...
- [x] Step 3: Array literals (parser + compiler + VM) — added AST_ARRAY node, `[` prefix parsing with trailing commas, OP_ARRAY opcode, compiler/VM/disassembler support, 7 parser tests + 4 eval tests
- [x] Step 4: Array indexing (read + write) — added AST_INDEX/AST_INDEX_ASSIGN nodes, infix `[` parsing (prec 10), OP_INDEX_GET/OP_INDEX_SET opcodes, COW write-back for variable index assignment, 5 parser tests + 12 VM tests
- [x] Step 5: Array concatenation with `++` — extended OP_CONCAT to handle array-array concat (new array with all elements from both), mixed array/non-array error, 8 VM tests
- [x] Step 6: Built-in array functions (`len`, `push`, `pop`) — added native_len/native_push/native_pop in vm.c, registered in register_builtins(), 17 VM tests
- [x] Step 7: Integration + REPL + cleanup — added 9 array equality VM tests, 12 CLI integration tests (REPL + cutlet run + debug flags), fixed multiline array literals with skip_newlines() in parser

## Summary

All 7 steps complete. Arrays are fully implemented with value semantics, COW, structural equality, and multiline support.

**Files touched across all steps:**
- `src/tokenizer.c`, `src/tokenizer.h` — `[`, `]` as solo symbols
- `src/value.c`, `src/value.h` — VAL_ARRAY, ObjArray, refcounting, COW, formatting, equality
- `src/parser.c`, `src/parser.h` — AST_ARRAY, AST_INDEX, AST_INDEX_ASSIGN, multiline array support
- `src/chunk.c`, `src/chunk.h` — OP_ARRAY, OP_INDEX_GET, OP_INDEX_SET opcodes + disassembly
- `src/compiler.c` — array literal, index read, index write compilation
- `src/vm.c` — array execution, indexing, COW index assignment, concat, len/push/pop builtins, equality
- `tests/test_tokenizer.c`, `tests/test_value.c`, `tests/test_parser.c`, `tests/test_vm.c`, `tests/test_cli.sh` — comprehensive tests
