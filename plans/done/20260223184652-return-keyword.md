...
- [x] Step 3: Add compile_return to the compiler — added compile_return function and AST_RETURN case in compile_node; all 322 VM tests pass, make check clean
- [x] Step 4: Add example program — added examples/return.cutlet and examples/return.expected; all 21 example tests pass

## Summary

Added the `return` keyword for early function exit. `return expr` exits the enclosing function with a value; bare `return` returns `nothing`. `return` at the top level is a compile error.

**Files touched:**
- `src/parser.h` — added `AST_RETURN` enum value
- `src/parser.c` — reserved keyword, parse block, formatting, keyword exclusion lists
- `src/compiler.c` — added `compile_return` function and `AST_RETURN` case
- `tests/test_parser.c` — 4 parser tests for return expressions
- `tests/test_vm.c` — 11 VM integration tests for return behavior
- `examples/return.cutlet` — example program demonstrating return
- `examples/return.expected` — expected output for example
