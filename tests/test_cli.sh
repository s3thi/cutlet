...
# --ast output contains OBJECT_DEF and MAKE nodes
ast_obj_tmpfile=$(mktemp /tmp/cutlet_test_XXXXXX)
printf '%s' 'object Foo is fn x(self) is "hi" end end
make Foo().x()' > "$ast_obj_tmpfile"
ast_obj_result=$("$CUTLET" run "$ast_obj_tmpfile" --ast 2>/dev/null)
if echo "$ast_obj_result" | grep -qF "OBJECT_DEF" && echo "$ast_obj_result" | grep -qF "MAKE"; then
    echo "  PASS: --ast shows OBJECT_DEF and MAKE"
    PASS=$((PASS + 1))
else
    echo "  FAIL: --ast shows OBJECT_DEF and MAKE"
    echo "    Expected to contain: OBJECT_DEF and MAKE"
    echo "    Got: $ast_obj_result"
    FAIL=$((FAIL + 1))
fi
rm -f "$ast_obj_tmpfile"

# --bytecode output contains OP_OBJECT_TYPE and OP_MAKE
bc_obj_tmpfile=$(mktemp /tmp/cutlet_test_XXXXXX)
printf '%s' 'object Foo is fn x(self) is "hi" end end
make Foo().x()' > "$bc_obj_tmpfile"
bc_obj_result=$("$CUTLET" run "$bc_obj_tmpfile" --bytecode 2>/dev/null)
if echo "$bc_obj_result" | grep -qF "OP_OBJECT_TYPE" && echo "$bc_obj_result" | grep -qF "OP_MAKE"; then
    echo "  PASS: --bytecode shows OP_OBJECT_TYPE and OP_MAKE"
    PASS=$((PASS + 1))
else
    echo "  FAIL: --bytecode shows OP_OBJECT_TYPE and OP_MAKE"
    echo "    Expected to contain: OP_OBJECT_TYPE and OP_MAKE"
    echo "    Got: $bc_obj_result"
    FAIL=$((FAIL + 1))
fi
rm -f "$bc_obj_tmpfile"

# Error case: make on a non-type value produces an error
test_run_file_error "make non-type error" 'make 42()' "expected type name"
